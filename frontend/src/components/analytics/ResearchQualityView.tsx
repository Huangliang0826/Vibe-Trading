...
export function ResearchQualityView({ days }: { days: AnalyticsDays }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true);
    const isRunSubject = subject === "backtest" || subject === "paper_trading";
    const selectedHorizon = isRunSubject
      ? "run"
