...
import { Loader2, Zap, AlertTriangle, Clock, History } from "lucide-react";
import { api, type PaperTickState, type PaperScheduleState, type PaperAction } from "@/lib/api";
...
export function PaperAutoTradePanel({ halted, onAfterExecute }: { halted: boolean; onAfterExecute?: () => void }) {
  ...
  const start = async () => {
    ...
    setBusy(true);
    try {
      const s = await api.runPaperTick(false);
