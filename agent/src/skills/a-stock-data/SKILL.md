...
description: A股全栈数据工具包 — 行情(mootdx+腾讯+百度)/研报(东财+同花顺)/新闻(东财)/基础数据(东财+新浪)/公告(巨潮) + 信号/资金面。免 API key；东财接口经 em_get 限流防封。A 股个股估值、研报检索、新闻、财报、公告、龙虎榜、北向、解禁等场景加载本 skill。
---

> **Vibe-Trading 内置集成（先读）**
>
> 本 skill 的 7 层配方已部分固化为可直接 import 的代码，零网络环境下的单测见 `agent/tests/test_a_stock_data_*`：
>
> - **行情 / OHLCV**：已注册为回测数据源 `a_stock_data`（A股回退链首位
>   `[a_stock_data, tushare, mootdx, akshare]`）。回测 config 写 `"source": "a_stock_data"`
>   或 `"auto"` 即可；实现见 `backtest/loaders/a_stock_data.py`（百度日K为主、mootdx 兜底）。
> - **研报 / 新闻 / 基础数据 / 公告 / 实时估值**：见
>   `backtest/loaders/a_stock_data_research.py`，可直接调用：
>   `fetch_reports(code)` · `fetch_news(code=None)` · `fetch_stock_info(code)` ·
>   `fetch_financials(code, report_type)` · `fetch_announcements(code)` · `fetch_quote(codes)`。
>   所有东财调用共用 `_a_stock_data_common.em_get` 进程级限流。
> - **其余层（信号/资金面/iwencai 等 27 端点）**：尚未固化，按下文配方现写现用即可。
>
> **运行依赖**：`uv sync --extra a-stock`（mootdx + stockstats；requests/pandas 已在核心依赖）。
> **网络要求**：所有数据源指向中国大陆服务器（通达信/东财/同花顺/新浪/巨潮/腾讯/百度），
