...
支持买入持有、定投、网格、动量突破、移动止损、风险平价等 **30 多种策略**和稳健性优化，可配置多市场投资组合、回测区间与初始资金，并展示净值、回撤、分标的统计和交易明细。新增的**资产管理**可从三市场自选加入候选资产，使用 `DEEPSEEK_API_KEY` 直连 DeepSeek 官方 API，由 DeepSeek V4 Pro 根据预期收益、最大回撤及历史风险数据直接生成目标配比；本地只做资产与权重合法性校验和事后风险测算，最近一次成功方案会持续保存到下次计算。
