...
默认的**历史回测**保留买入持有、定投、网格、动量突破、移动止损、风险平价等 **30 多种策略**和稳健性优化；新增的**策略比较**使用同一区间、初始资金与交易成本，对照 SPY 买入持有、SPY / 现金 200 日均线轮动和防守型个股动量 Strategy V0。支持近 1 / 3 / 5 / 10 年及自定义区间，默认过去五年至今天，并展示净值、回撤、现金比例、年度收益和验证记分卡。

Strategy V0 的信号在收盘后计算、下一交易日开盘执行，现金收益率按 0% 计算。当前个股面板使用现有标普成分股，因此结果明确标记**幸存者偏差**，只能用于策略初筛，不能替代历史时点成分股或样本外正式验证。
