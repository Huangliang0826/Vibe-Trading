...
支持买入持有、定投、网格、动量突破、移动止损、风险平价等 **30 多种策略**和稳健性优化，可配置多市场投资组合、回测区间与初始资金，并展示净值、回撤、分标的统计和交易明细。新增的**资产管理**可从三市场自选加入股票、ETF 与债券基金，所有资产类型、资产比例和现金比例均由用户手动配置并自动保存；回测与 10 万美元虚拟追踪都先分十周建仓，再从建仓完成日起每三个月将全部资产与现金恢复至目标比例。追踪数据使用 SQLite WAL、独立再平衡批次、现金账本、幂等成交记录和每日估值保存。
