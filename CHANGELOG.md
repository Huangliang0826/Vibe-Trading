...
All notable changes to Alpha Mind are documented in this file.
This project adheres to [Semantic Versioning](https://semver.org/).

## [Unreleased]

### Changed
- **走势预测风险指标。** 最稳健策略及样本外结果分别展示相对初始本金的“最大亏损”和峰值到谷值的“最大回撤”；后端响应保留 `max_loss`，不再把稳健窗口的平均最大亏损误标为最大回撤。
- **本地启动与健康检查。** `scripts/dev doctor` 统一检查依赖、PID、固定端口、前后端与 Vite API 代理；启动脚本清理失效 PID、识别残留端口并验证完整代理链路。页面独立监测后端健康状态，断线时显示可重试的中文提示，API 误返回 HTML 时不再展示原始页面内容。
